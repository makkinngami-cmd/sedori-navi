...
- 次フェーズは、買取一丁目の商品追加候補作成と、買取一丁目価格アラームの設計・実装。

### 次にやること

- 買取一丁目の商品一覧をスクレイピングし、未登録商品候補表を作る。
- 候補表ではJANあり商品を優先し、JAN、買取一丁目商品名、カテゴリ、買取価格、既存登録有無、追加優先度を見える化する。
- 候補表を見て、正式に追加する商品を決める。
- 買取一丁目だけを対象にした価格アラーム仕様を検討する。
- アラーム条件は画面で変更できるようにし、「％かつ円」「％または円」「％のみ」「円のみ」を扱う。
