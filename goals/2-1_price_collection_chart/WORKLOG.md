...
- 日次スクレイピングは6/11〜6/15連続稼働を確認。6/13のみヤフオクが途中終了していたが、6/14以降は正常。
- `トモダチコレクション Switch 2` は全店取得0件が継続中。未発売のため対処不要と判断。
- 価格アラーム `docs/alerts.html` に7日以内制限を追加。古い値上がりがアラームに出続ける問題を解消。
- 価格アラームにカテゴリフィルタータブ（ゲーム/カメラ/ポケカ/ワンピ/スマートフォン/その他）とお気に入りフィルターを追加。index.htmlと同じlocalStorageを共有。
- カメラ買取一丁目のみ8件をウォッチ。X-M5シルバーのみヤフオク落札出現（¥149,000 vs 買取¥113,000）。他7件は変化なし。
- PS5ソフトのヤフオク低額混入を確認。`scrape_yahoo.py` のマッチングフィルタに `min_price` チェックを追加（URLパラメータだけでは弾けないケースへの二重チェック）。混入15行を `prices.csv` から削除。
...
- iPhone 17 無印 256GB/512GB 各5色（セージ/ブラック/ホワイト/ミストブルー/ラベンダー）。定価はApple Japan MSRP 256GB=129,800 / 512GB=164,800（price_type空=買取率表示）。color別JAN付きで登録。
- FUJIFILM X-T50 ボディ 黒/銀、X-E4 ボディ 黒/銀・レンズキット 黒/銀。price_type=market（買取一丁目候補価格を市場参考）。
- products.py（スマホ/カメラ）、index.html・alerts.html の PRODUCT_CATEGORIES、data/docs両方の msrp.csv を更新。
- 強制スクレイプで16商品すべて当日買取データ取得を確認（iPhoneはJAN一致でマッチ。X-T50買取¥176,000/¥188,000、X-E4¥135,000〜143,900、iPhone17 256GB¥129,000/512GB¥148,500）。
- 未登録の派生モデルも確認（X-T50 XC15-45/XF16-50レンズキット等）。強化中なので次回追加候補。

## 2026-06-28 買取強化中リサーチの仕組み

- 買取一丁目トップの「買取強化中」は専用API `/api/index/getImpoProduct`（ログイン不要・ページング）で取得できることを特定。スクショの強化商品と並び一致で確認。
- 新スクリプト `scraper/generate_ichome_boost_candidates.py` を追加。強化中リスト全件を取得し、JAN/名前で既存商品と照合、未登録のものを `reports/ichome_boost_candidates.csv/.md` に出力。携帯はキー帯のカラー展開、それ以外はjan/priceで処理。
- 初回実行: 強化中537件取得、既存登録100件、未登録の新規候補437件（その他261/スマホ147/カメラ18/ゲーム10/ポケカ1）。
- `run_scraper.ps1` に週1（毎週月曜）で実行するブロックを追加。レポートはcoverage同様commit/pushされる。
- 商品の `products.py` 等への追加自動化（オプション2の機械挿入）は、キーワード生成・JAN重複・MSRP補完で誤りが出やすく危険と判断。運用は「週1レポートで洗い出し→ユーザーが選んだ分だけClaudeが3ファイルへ追記」のバッチ方式とする。

## 2026-06-28 競合比較（買取サーチ）

ユーザーが類似サイトを発見。比較を実施。
- 競合1: https://kaitori-search.app/ （買取サーチβ — 買取価格比較サイト）
- 競合2: https://applepickup.kaitori-search.app/ （iPhone 17在庫モニター。買取サーチの別機能）

### サイトの性格の違い
- 自社 sedori-navi: せどり判断ツール。仕入れ（買取価格）＋売却（ヤフオク相場）の両面で利ざやを見る。
- 競合 買取サーチ: 買取価格の比較サイト。売却側（どの店に売れば高いか）のみ。ヤフオク相場は扱わない。

### 機能比較サマリー
- 買取店舗数: 自社6店 / 競合 約30店（競合が圧倒的）
- ヤフオク相場（売値）: 自社◯ / 競合✕ ← 自社の最大の差別化
- 価格の時系列チャート: 自社◯（日次・期間切替）/ 競合△（週次トレンドのみ）
- 価格アラーム: 自社◯ / 競合△（前日比変動表示）
- 買取率・市場比（MSRP）: 自社◯ / 競合✕
- カテゴリ数: 自社6 / 競合10以上（タブレット/PC/スマートウォッチ/イヤホン等）
- バーコード検索: 両方◯
- 実店舗検索: 自社✕ / 競合◯
- まとめ売り試算（カート）: 自社✕ / 競合◯
- 共通の買取店: 森森買取、買取一丁目（同じ店を見ているが、自社はヤフオク売値を持つ分、情報が一段上）

### 競合の強みへの対応方針（提案）
1. 店舗数30: 網羅数では追わない。「取得1店のみ商品」を減らす方向で、競合の未導入店（海峡通信・モバイルミックス等）を2〜3店だけ評価して追加。優先度中。
2. カテゴリの広さ: 安易に広げない。利ざや・回転率が見込める品目だけ厳選。既存6カテゴリの深掘り優先。優先度低。
3. まとめ売り試算（カート）: 単体実装は見送り。在庫管理（2-2）と統合し「手持ち在庫の合計売却額」として自社文脈で活かす方向。優先度低。
4. 実店舗検索: せどりはオンライン中心のため対象外。

結論: 機能数で張り合わず、「ヤフオク相場×履歴チャート×買取率」という自社の核を強化。店舗は数より既存商品のカバレッジを埋める質的拡張に絞る。
