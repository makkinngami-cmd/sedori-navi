...
詳細な作業指示・引き継ぎ・作業ログは `goals/` 配下を参照する。

```text
goals/
  README.md
  2-1_price_collection_chart/SESSION.md
  2-2_inventory_integration/SESSION.md
```

親フォルダの `STATUS.md` は朝のNotionレポート用サマリーとして使う。長い作業ログや詳細手順は `goals/{goal}/HANDOFF.md` / `WORKLOG.md` に書く。
...
**ゴール**：🔄（進行中）と ⬜（未着手）のみ書く。✅（完了）は翌日には削除する。機能単位の大きな粒度で書く。新しいゴールが決まったときは粒度を気にせずそのまま書く（粒度調整は朝の確認セッションが行う）。

**次にやること**：`タスク ／ いつ・何待ち` の形で書く（／で必ず区切る）。**太字**などの装飾は入れない。

**タイムスケジュール**：`- 時刻または契機 | 内容 | 担当` の形で書く。担当は `あなた` / `Claude` / `タスクスケジューラ` のいずれか。
